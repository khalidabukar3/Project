...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        // Make temp variable to hold initial value of array[i].
        T temp = array[i];
        // Replace both array entries appropriately.
        array[i] = array[j];
        array[j] = temp;
    }
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        // Define indexes of left and right entries.
        int left = 2 * top + 1;
        int right = left + 1;

        /*
         * If left does not exceed last entry to siftDown to, and right does
         * exceed, and if the top entry is greater than the left entry, then
         * switch top with left.
         */
        if (left <= last) {
            if (right > last) {
                ...
                /*
                 * Otherwise, define leftLessThanRight
                 */
            } else {
                boolean leftLessThanRight = order.compare(array[left],
                        array[right]) < 0;
                if (order.compare(array[top], array[left]) > 0
                        && leftLessThanRight) {
                    /*
                     * If top is greater than left, and left is smaller than
                     * right, then exchange top and left, and sift left down to
                     * the appropriate place.
                     */
                    exchangeEntries(array, top, left);
                    siftDown(array, left, last, order);
                } else if (order.compare(array[top], array[right]) > 0
                        && !leftLessThanRight) {
                    /*
                     * Else if top is greater than right, and right is smaller
                     * than left, then exchange top and right, and sift right
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * If left does not exceed array length, then heapify, using left as new
         * top.
         */
        ...
        /*
         * If right does not exceed array length, then heapify, using right as
         * new top.
         */
        ...
        /*
         * If left and right both do not exceed array length, then siftDown,
         * using array.length-1 as new last.
         */
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        int i = 0;
        /*
         * Loop while q is not empty, and reference each dequeue from q onto a
         * new index of the heap.
         */
        while (q.length() > 0) {
            heap[i] = q.dequeue();
            i++;
        }
        // Heapify the non-heap, starting from index 0. Then, return it.
    ...
    private void createNewRep(Comparator<T> order) {
        /*
         * Following basic defaults (insertionMode = true, use provided order,
         * start with empty queue, heapSize = 0).
         */
    ...
    public final void changeToExtractionMode() {
        assert this.isInInsertionMode() : "Violation of: this.insertion_mode";
        // Insertion mode instance variable set to false.
        this.insertionMode = false;
        // Set heapSize to length of queue.
        this.heapSize = this.entries.length();
        // Build heap using queue entries and order.
    ...
    public final T removeFirst() {
        ...
        // Storing value of first entry.
        T removed = this.heap[0];

        // Exchange the first entry with the last entry (larger than the root).
        ...
        /*
         * Sift down the entry at 0 all the way to the bottom, since you just
         * exchanged it.
         */
    ...
    public final int size() {
        ...
        if (this.isInInsertionMode()) {
            // If in insertionMode, then size is length of entries.
            size = this.entries.length();
        } else {
            // Otherwise, size is value of heapSize.
            size = this.heapSize;
        }
        assert this.conventionHolds();
        return size;
